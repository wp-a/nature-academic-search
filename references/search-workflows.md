...
4. 含汉字的问题不要整句丢给英文元数据源。先抽出拉丁词（如 AI），用 `lookup_mesh` 或英文同义词构成检索式；`search_run.query_analysis.mesh_required` 为 true 时必须披露这一步。本地 `ranking` 会对中文双字和英文词分别打分，但不能把中文查询翻译成 MeSH。
5. 保存人类可读的原查询。PubMed 字段语法不能原样复制给其他来源。
6. 只在用户要求或范围需要时添加日期、类型和语言过滤。
